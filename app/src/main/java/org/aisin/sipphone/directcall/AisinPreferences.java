 ...
 */
public class AisinPreferences {
	private static final int LINPHONE_CORE_RANDOM_PORT = -1;
	private static AisinPreferences instance;
	private Context mContext;

	public static final synchronized AisinPreferences instance() {
		if (instance == null) {
			instance = new AisinPreferences();
		}
		return instance;
	}

	private AisinPreferences() {
	...
	private String getString(int key) {
		if (mContext == null && AisinManager.isInstanciated()) {
			mContext = AisinManager.getInstance().getContext();
		}
	...
	private LinphoneCore getLc() {
		...
		return AisinManager.getLcIfManagerNotDestroyedOrNull();
	}
	...
	public LpConfig getConfig() {
		...
		if (!AisinManager.isInstanciated()) {
			Log.w("AisinManager not instanciated yet...");
			return LinphoneCoreFactory.instance().createLpConfig(mContext.getFilesDir().getAbsolutePath() + "/.linphonerc");
		}

		return LinphoneCoreFactory.instance().createLpConfig(AisinManager.getInstance().mLinphoneConfigFile);
	}
	...
	public static class AccountBuilder {
		...
		public void saveNewAccount() throws LinphoneCoreException {
			...
			if (!tempNoDefault && AisinPreferences.instance().getAccountCount() == 1)
	...
	public void setTunnelHost(String host) {
		...
		if(config != null) {
			config.setHost(host);
			AisinManager.getInstance().initTunnelFromConf();
		}
	}
