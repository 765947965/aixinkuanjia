...
public class NumberCircleProgressBar extends View {
	...
	public void calculateDrawLoad(String text) {
		 invalidate();
		mCurrentDrawText ="加载中"+"\n"+"...";
		mCurrentDrawText = mPrefix + mCurrentDrawText ;
		Rect rect = new Rect();
		// ���ذ�Χ����ַ����С��һ��Rect����
		mTextPaint.getTextBounds(mCurrentDrawText, 0, 1, rect);
		mDrawTextWidth = rect.width();
	...
	public void setProgress(int Progress) {
		...
		if (Progress <= getMax() && Progress >=10) {
			this.mProgress = Progress-lastNum;
			invalidate();
