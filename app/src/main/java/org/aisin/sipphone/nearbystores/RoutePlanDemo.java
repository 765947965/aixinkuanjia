...
public class RoutePlanDemo extends Activity implements
	...
	private LinearLayout routeplandemo_linlayout;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		st_longitude = getIntent().getDoubleExtra("RoutePlanDemo.st_longitude",
				0);
		en_latitude = getIntent()
				.getDoubleExtra("RoutePlanDemo.en_latitude", 0);
		en_longitude = getIntent().getDoubleExtra("RoutePlanDemo.en_longitude",
				0);
		if (st_latitude == 0 && st_longitude == 0 && en_latitude == 0
				&& en_longitude == 0) {
			finish();
		}
		routeplandemo_linlayout = (LinearLayout) this
				.findViewById(R.id.routeplandemo_linlayout);
		...
		mBaidumap.setMapStatus(MapStatusUpdateFactory
				.newMapStatus(new MapStatus.Builder().zoom(13).target(
						new LatLng(st_latitude, st_longitude)).build()));

		
		// 驾车线路搜索
		PlanNode stNode = PlanNode.withLocation(new LatLng(st_latitude,
				st_longitude));
		PlanNode enNode = PlanNode.withLocation(new LatLng(en_latitude,
				en_longitude));
	...
	protected void onDestroy() {
		...
		mMapView.onDestroy();
		super.onDestroy();
		RecoveryTools.unbindDrawables(routeplandemo_linlayout);// 回收容
	}
