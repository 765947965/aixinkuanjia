...
public class MobileCardRecharge extends Activity implements OnClickListener,
	...
	public void onClick(View v) {
		...
		case R.id.prepaid_card_bt:
			...
			if (prepaid_card_num.getText().toString().trim().length() < 6
					|| !Check_format.Check_num(prepaid_card_num.getText()
							.toString().trim())) {
				Toast.makeText(this, "请输入正确的充值卡序列号!", Toast.LENGTH_SHORT)
						.show();
				return;
			}
			if (prepaid_card_password.getText().toString().trim().length() < 6
					|| !Check_format.Check_num(prepaid_card_password.getText()
							.toString().trim())) {
