...
public class GroupRedEnvelopeConfig extends Activity implements OnClickListener {
	...
	private TextView reduction_rednum;// 减红包的数量
	private TextView add_rednum;// 加红包的数量
	private EditText red_group_num;// 红包的数量
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.groupredenvelopeconfig);
		...
		reduction_rednum = (TextView) this.findViewById(R.id.reduction_rednum);
		add_rednum = (TextView) this.findViewById(R.id.add_rednum);
		red_group_num = (EditText) this.findViewById(R.id.red_group_num);
		reduction_rednum.setOnClickListener(this);
		add_rednum.setOnClickListener(this);
	...
	public void onClick(View v) {
		...
		case R.id.sendpalpcredbt:
			...
			if (("".equals(whoimisedit.getHint().toString()) || "请输入姓名"
				...
				new AisinBuildDialog(GroupRedEnvelopeConfig.this, "提示",
						"请输入姓名!");
			} else if (money_input.getText().toString().equals("")
					|| money_input.getText().toString().equals("0")) {
				new AisinBuildDialog(GroupRedEnvelopeConfig.this, "提示",
						"请输入金额!");
			} else if (red_group_num.getText().toString().equals("")
					|| red_group_num.getText().toString().equals("0")) {
				new AisinBuildDialog(GroupRedEnvelopeConfig.this, "提示",
						"请输入红包数量!");
			} else {
				try {
			...
			break;
		case R.id.reduction_rednum:// 减红包
			String temp = red_group_num.getText().toString();
			int tempint = 0;
			if (!"".equals(temp)) {
				tempint = Integer.parseInt(temp) - 1;
				if (tempint < 0) {
					red_group_num.setText("0");
				} else {
					red_group_num.setText(tempint + "");
				}
			} else {
				red_group_num.setText(tempint + "");
			}
			break;
		case R.id.add_rednum:// 加红包
			String temp2 = red_group_num.getText().toString();
			int tempint2 = 1;
			if (!"".equals(temp2)) {
				tempint = Integer.parseInt(temp2) + 1;
				red_group_num.setText(tempint + "");
			} else {
				red_group_num.setText(tempint2 + "");
			}
			break;
		}
	}
	...
	class HttpTask_sendRed extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... params) {
			...
			String url = URLTools.GetHttpURL_4SendGroupRed(
					GroupRedEnvelopeConfig.this, red_group_num.getText()
							.toString(), usinfo.getUid() + "", gift_type,
					money_input.getText().toString().trim() + "00", money_type,
					temp, zfy, fromname);
		...
		protected void onPostExecute(String result) {
			...
			case 0:
				try {
					...
					RedData_DBHelp.addRedDatas(GroupRedEnvelopeConfig.this,
							redobject);

					Intent intent = new Intent(GroupRedEnvelopeConfig.this,
					...
					intent.putExtra("sended_gift_id", redobject.getGift_id());
					GroupRedEnvelopeConfig.this.startActivity(intent);
					GroupRedEnvelopeConfig.this.finish();
			...
			case 45:
				...
				break;
			case 19:
				new AisinBuildDialog(GroupRedEnvelopeConfig.this, "提示", "余额不足!");
				break;
			default:
				new AisinBuildDialog(GroupRedEnvelopeConfig.this, "提示",
						"联网失败，请检查您的网络信号！");
	...
	private class HttpTask_accountquery extends
		...
		protected String doInBackground(String... paramArrayOfParams) {
			// 获取查询URL
			String url = URLTools
					.GetHttpURL_4Accountquery_YE(GroupRedEnvelopeConfig.this);
			String result = HttpUtils.result_url_get(url, "{'result':'-14'}");
			return result;
		}
